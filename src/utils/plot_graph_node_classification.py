...
def plot(hits, edge_matrix, y):
    ...
    signal_color = ['#2b2b2b', 'blue', 'orange', 'red', 'purple', 'green', 'brown', 'magenta', 'cyan', 'yellow', 'black'] # colors for different turns
...
def plot_only_sev_id(hits, edge_matrix, y, sevid = 27):
    """
    Plot the graph based on an edge matrix of shape 2 x num_edges
    containing hit ID connected
    """
    ...
    for k, e in enumerate(edge_matrix.T):
        ...
        if(((hits[i, 8] == sevid ) or (hits[i, 8] ==-1)) and ((hits[j, 8] == sevid ) or (hits[j, 8] ==-1))):
            plt.plot([xi, xj], [yi, yj], color='grey', linewidth=0.5, linestyle='-.', alpha=.25)
    plt.draw()  # Ridisegna la figura mantenendo i punti già plottati
...
def plot_clusters_turns(hits, edge_matrix, y):
    ...
    #select hits based on truth


    turn_1_index = np.where(y==1)
    turn_2_index = np.where(y==2)
    turn_3_index = np.where(y==3)
    
    
    hits_turn1 = hits[turn_1_index]            
    hits_turn2 = hits[turn_2_index]    
    hits_turn3 = hits[turn_3_index]


    list_turns = [hits_turn1, hits_turn2, hits_turn3]
    
    
    for iteration, item in enumerate(list_turns):
        # Draw CDCH scheleton
        num_circles = 10
        min_radius = 17
        max_radius = 25
        ...
        for i in range(num_circles):
            ...
            if i == 0 or i == num_circles - 1:
                ...
                linewidth = 0.5
            draw_circle(radius, linewidth)
        for i, hit in enumerate(item):
            plt.errorbar(hit[0], hit[1], fmt=fmts[round(hit[7])], alpha=.6, markersize=10, color=signal_color[int(iteration+1)])
