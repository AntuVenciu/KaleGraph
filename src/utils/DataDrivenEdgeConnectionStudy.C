...
void DataDrivenEdgeConnectionStudy()
{
  ...
  // This matrix has dimension NUM_NODES x NUM_NODES.
  // NUM_NODES is the total number of CDCH wires and TC tiles
  // Each entry (i, j) register the number of times node-j 
  // follows the node-i in tracking.
  // pTC tiles are numbered in the matrix as tileID + 1920
  vector<vector<int>> EdgeMatrix;
  int num_nodes = 1920 + 512; // cdch wires + tc tiles
  ...
  TBranch *bSPXHit;
  TClonesArray *pSPXHitArray = new TClonesArray("MEGDCHHit");
  TBranch *bDCHTrack;
  ...
  for (Int_t iEvent = 0; iEvent < nEvent; iEvent++) {
	...
	c->Update();
      */
      ...
      // Filling EdgeMatrix
      for (int id=0; id < ngoodhits - 1; id++) {
	EdgeMatrix.at(wireID_sorted.at(id)).at(wireID_sorted.at(id + 1))++;
      }
      
      // Fill TC part of the Edge Matrix 
      
      Int_t nhits_spx = pSPXTrack->Getnhits();
      vector<double> zhit_spx_vec;
      vector<double> phihit_spx_vec;
      vector<Int_t> pixelID_vec;
      
      Int_t ngoodhits_spx = 0;
	...
	Int_t pixelID = aHit->Getpixelid();
	pixelID_vec.push_back(pixelID);
	Float_t zhit = aHit->GetxyzAt(2);
	//double phihit = state->GetPhi();
	zhit_spx_vec.push_back(zhit);
	//phihit_vec.push_back(phihit);
	ngoodhits_spx++;
      }

      // Sorting
      vector<int> pixelID_sorted = wireIDSorting(zhit_spx_vec, pixelID_vec);
      vector<double> z_sorted_spx = zSorting(zhit_spx_vec, pixelID_vec);
      //vector<double> phi_sorted = phiSorting(phihit_vec, zhit_vec);
	...
	c->Update();
      */
      ...
      // Filling EdgeMatrix
      for (int id=0; id < ngoodhits_spx - 1; id++) {
	//cout << pixelID_sorted.at(id) + 1920 << endl;
	EdgeMatrix.at(pixelID_sorted.at(id) + 1920).at(pixelID_sorted.at(id + 1) + 1920)++;
      }
      
      // Fill mixed CDCH - TC part of the EdgeMatrix
      // Find hit in CDCH before TC and TC tile ID to fill TC part of the EdgeMatrix
      int matchingPoints = pDCHSPXMatchedTrack->GetMatchedPixelIdSize();
      if (pDCHSPXMatchedTrack->GetMatchedPixelIdSize() != pDCHSPXMatchedTrack->GetmatchedDCHHitIndexInTrackSize()) {
	cout << "No matches in hit and pixel ID" << endl;
	continue;
      }
      for (int ihit=0; ihit<matchingPoints; ihit++) {
	Short_t dch_hit_index = pDCHSPXMatchedTrack->GetmatchedDCHHitIndexInTrackAt(ihit);
	MEGDCHHit* aDCHHit = (MEGDCHHit*)pDCHHitArray->At(dch_hit_index);
	Int_t wireID = aDCHHit->Getwire();
	// Find cluster
	Short_t spx_hit_index = pDCHSPXMatchedTrack->GetMatchedPixelIdAt(ihit);
	MEGSPXHit* aSPXHit = (MEGSPXHit*)pSPXHitArray->At(spx_hit_index);
	Short_t spxID = aSPXHit->Getpixelid();
	// Fill the EdgMatrix
	EdgeMatrix.at(wireID).at(1920 + spxID)++;
	EdgeMatrix.at(1920 + spxID).at(wireID)++;
